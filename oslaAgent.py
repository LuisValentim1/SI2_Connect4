...
class RLAgent:
    ...
    def evalBoard(self, board, move, player):
        simBoard = copy.deepcopy(board)
        simBoard.placePiece(move)
        simBoard.checkWinner()
        if simBoard.winner == player:
            return 10000

        if self.blockedSeq3(board, (3%player)+1, move):
            return 1000

        if self.madeSeq3(board, simBoard, player):
            return 100

        if self.madeSeq(board, simBoard, player):
            return 10 
    ...
    def madeSeq3(self, board, simulatedBoard, player):
        if(len(board.getSequencesFromPlayerWithLength(self, 3, player)) < len(simulatedBoard.getSequencesFromPlayerWithLength(self, 3, player))):
            return True

    def madeSeq(self, board, simulatedBoard, player):
        if(len(simulatedBoard.getSequencesFromPlayer(self, player))-len(board.getSequencesFromPlayer(self, player)) == 2 ):
            return True
